...
def get_real_num_rows (input_pcap):
    ...
    for (pkt_data, pkt_metadata,) in RawPcapReader (input_pcap):
        ...
        if ether_pkt.dst != "ec:1a:59:79:f4:89" and ether_pkt.src != "ec:1a:59:79:f4:89":
            continue
...
def plotEntropy (df, attack_df):
    ...
    plt.plot(np.arange (1, episod_limit + 2, 1), thresholds, marker = 'None', linestyle = '-', color = "tab:orange", label = 'Entropy')
    plt.xlabel('Time')
    plt.ylabel('Entropy')
    plt.grid()
    plt.title("(Dst + Src) IP")
    plt.legend(loc='best')
